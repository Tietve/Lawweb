...
export class ZaloClient {
    ...
    accessToken = null;
    tokenExpiry = 0;
    constructor(_appId, _secretKey, _oaId, kv) {
        this._appId = _appId;
     ...
     */
    async sendQuickReply(userId, message, options) {
        await this.ensureValidToken();
    ...
    async getUserInfo(userId) {
        await this.ensureValidToken();
        const response = await fetch(`${this.baseUrl}/oa/getprofile?data=${JSON.stringify({ user_id: userId })}`, {
            method: 'GET',
            headers: {
                access_token: this.accessToken,
            },
    ...
    async refreshAccessToken() {
        ...
        throw new Error('Token refresh not implemented. Please provide access token via KV or implement OAuth flow.');
    }
    /**
     ...
     */
    async setAccessToken(token, expiresIn = 7200) {
        this.accessToken = token;
        this.tokenExpiry = Date.now() + expiresIn * 1000;
        // Cache in KV if available
        if (this.kv) {
            await this.kv.put('zalo_access_token', JSON.stringify({
                token,
 ...
 */
export function createZaloClient(appId, secretKey, oaId, kv) {
