...
import type { ZaloWebhookEvent, QueueMessage } from '@lawbot/shared';
...
export async function handleZaloWebhook(
  ...
  try {
    ...
    // Verify signature
    const isValid = await verifyZaloSignature(
      body,
      signature,
      c.env.ZALO_APP_SECRET
    );

    if (!isValid) {
      console.warn('Invalid signature');
      await ZaloMonitor.logWebhookEvent(
        'user_send_text',
        undefined,
        { error: 'Invalid signature', ip: monitoringCtx.ip },
        c.env.DB
      );
    ...
    // Parse webhook event
    const event: ZaloWebhookEvent = JSON.parse(body);
...
async function handleUserFollow(
  ...
  await env.WEBHOOK_CACHE.put(
    `zalo_follow_${userId}_${Date.now()}`,
    JSON.stringify(queueMessage),
    {
      expirationTtl: 3600,
    }
...
async function handleUserUnfollow(
  ...
  await env.WEBHOOK_CACHE.put(
    `zalo_unfollow_${userId}_${Date.now()}`,
    JSON.stringify(queueMessage),
    {
      expirationTtl: 3600,
    }
