...
export class ZaloMonitor {
     ...
     */
    static async logWebhookEvent(eventType, userId, data, db) {
        try {
            await db
                .prepare(`
     ...
     * @param db - D1 database instance
     */
    static async logError(error, context, db) {
        try {
            await db
                .prepare(`
     ...
     */
    static async trackMetric(metric, value, labels) {
     ...
     */
    static async trackProcessingTime(startTime, eventType) {
 ...
 */
export async function ensureWebhookLogsTable(db) {
    await db
        .prepare(`
