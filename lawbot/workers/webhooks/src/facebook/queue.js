 ...
 */
export async function processFacebookQueue(batch, env) {
    ...
    for (const message of batch.messages) {
        ...
        try {
            ...
            if (message.attempts < 3) {
                ...
                console.log(`Retrying message, attempt ${message.attempts + 1}, delay: ${delaySeconds}s`);
            }
            else {
 ...
 */
async function processEntry(entry, client, env) {
 ...
 */
async function processMessagingEvent(event, client, env) {
 ...
 */
export async function processWebhookDirectly(body, env) {
    const client = new MessengerClient(env.MESSENGER_PAGE_ACCESS_TOKEN);
    if (body.object !== 'page') {
 ...
 */
export async function queueWebhookEvent(body, env) {
    if (!env.FB_QUEUE) {
