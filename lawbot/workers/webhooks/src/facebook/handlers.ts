...
import type { MessagingEvent, Env, Citation } from './types';
...
async function handleTextMessage(
  ...
  try {
    ...
    const response = await env.CHATBOT.fetch(
      new Request('http://chatbot/chat', {
        method: 'POST',
        headers: {
    ...
    await sendFormattedResponse(
      senderId,
      result.answer || 'Xin lỗi, tôi không thể xử lý câu hỏi này.',
      result.citations || [],
      client
    );
  } catch (error) {
    ...
    await client.sendTextMessage(
      senderId,
      'Cảm ơn câu hỏi của bạn. Hiện tại hệ thống đang trong giai đoạn phát triển. Vui lòng thử lại sau.'
    );
  }
}
...
async function handleAttachments(
  ...
  if (attachment.type === 'image' && attachment.payload.url) {
    await client.sendTextMessage(
      senderId,
      '📸 Cảm ơn bạn đã gửi hình ảnh. Hiện tại tôi chưa hỗ trợ phân tích hình ảnh, nhưng bạn có thể mô tả vấn đề pháp lý của mình bằng văn bản.'
    );
  } else if (attachment.type === 'location') {
    await client.sendTextMessage(
      senderId,
      '📍 Cảm ơn bạn đã chia sẻ vị trí. Bạn cần tư vấn pháp lý về vấn đề gì?'
    );
  } else {
    await client.sendTextMessage(
      senderId,
      'Tôi đã nhận được tệp đính kèm. Vui lòng mô tả vấn đề pháp lý của bạn bằng văn bản.'
    );
  }
}
  ...
  try {
    ...
    switch (payload) {
      ...
      case 'CUSTOM_QUESTION':
        await client.sendTextMessage(
          senderId,
          'Vui lòng nhập câu hỏi của bạn về pháp luật:'
        );
        break;
      ...
      default:
        ...
        await client.sendTextMessage(
          senderId,
          'Xin lỗi, tôi không hiểu yêu cầu này.'
        );
    }
  } catch (error) {
