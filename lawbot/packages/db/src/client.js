...
export class D1Client {
     ...
     */
    async batchInsert(table, records, batchSize = 10000) {
        if (records.length === 0)
            return;
    ...
    async archiveOldConversations(daysOld = 30) {
        ...
        const conversationsToArchive = await this.query(`SELECT id, user_id, platform, created_at, updated_at, metadata
       FROM conversations
       WHERE updated_at < ? AND status = 'active'`, [cutoffTimestamp]);
        if (conversationsToArchive.length === 0)
            return 0;
        const statements = [];
        for (const conv of conversationsToArchive) {
            // Get all messages for this conversation
            const messages = await this.query('SELECT * FROM messages WHERE conversation_id = ?', [conv.id]);
            ...
            statements.push(this.db
                .prepare(`INSERT INTO conversations_archive
             (id, user_id, platform, data, message_count, created_at, updated_at)
             VALUES (?, ?, ?, ?, ?, ?, ?)`)
                .bind(conv.id, conv.user_id, conv.platform, archiveData, messages.length, conv.created_at, conv.updated_at));
            // Delete messages
            statements.push(this.db.prepare('DELETE FROM messages WHERE conversation_id = ?').bind(conv.id));
