...
export class ConversationMemory {
    ...
    async saveMessage(conversationId, message) {
        try {
            ...
            await this.db
                .prepare(`INSERT INTO messages (conversation_id, role, content, created_at)
           VALUES (?, ?, ?, ?)`)
                .bind(conversationId, message.role, message.content, new Date().toISOString())
                .run();
            // Update KV cache
     ...
     */
    async updateCache(conversationId, newMessage) {
        try {
     ...
     */
    async getFromCache(conversationId) {
        try {
            const cacheKey = this.getCacheKey(conversationId);
            const cached = await this.kv.get(cacheKey, 'json');
            return cached || [];
     ...
     */
    async getFromDB(conversationId, limit) {
        try {
            const result = await this.db
     ...
     */
    async setCacheFromMessages(conversationId, messages) {
        try {
            const cacheKey = this.getCacheKey(conversationId);
