...
analytics.get('/dashboard', async (c) => {
    ...
    const newUsers = await c.env.DB.prepare('SELECT COUNT(*) as count FROM users WHERE created_at >= ?').bind(last30Days).first();
    // Total conversations
    const totalConversations = await c.env.DB.prepare('SELECT COUNT(*) as count FROM conversations').first();
    // Active conversations (last 24 hours)
    const activeConversations = await c.env.DB.prepare('SELECT COUNT(*) as count FROM conversations WHERE updated_at >= ?').bind(last24Hours).first();
    // Total messages
    const totalMessages = await c.env.DB.prepare('SELECT COUNT(*) as count FROM messages').first();
    // Messages today
    const today = new Date();
    today.setHours(0, 0, 0, 0);
    const messagesToday = await c.env.DB.prepare('SELECT COUNT(*) as count FROM messages WHERE created_at >= ?').bind(today.toISOString()).first();
    // Total documents
    const totalDocuments = await c.env.DB.prepare('SELECT COUNT(*) as count FROM documents').first();
...
analytics.get('/daily', validateQuery(dailyMetricsSchema), async (c) => {
    ...
    const { results: userMetrics } = await c.env.DB.prepare(`SELECT DATE(created_at) as date, COUNT(*) as count
     FROM users
     WHERE created_at >= ?
     GROUP BY DATE(created_at)
     ORDER BY date ASC`).bind(fromDate.toISOString()).all();
    // Daily conversations
    const { results: conversationMetrics } = await c.env.DB.prepare(`SELECT DATE(created_at) as date, COUNT(*) as count
     FROM conversations
     WHERE created_at >= ?
     GROUP BY DATE(created_at)
     ORDER BY date ASC`).bind(fromDate.toISOString()).all();
    // Daily messages
    const { results: messageMetrics } = await c.env.DB.prepare(`SELECT DATE(created_at) as date, COUNT(*) as count
     FROM messages
     WHERE created_at >= ?
     GROUP BY DATE(created_at)
     ORDER BY date ASC`).bind(fromDate.toISOString()).all();
...
analytics.get('/hourly', validateQuery(hourlyMetricsSchema), async (c) => {
    ...
    // Hourly messages
    const { results: messageMetrics } = await c.env.DB.prepare(`SELECT
       strftime('%Y-%m-%d %H:00:00', created_at) as hour,
       COUNT(*) as count
     FROM messages
     WHERE created_at >= ?
     GROUP BY hour
     ORDER BY hour ASC`).bind(fromDate.toISOString()).all();
    // Hourly conversations
    const { results: conversationMetrics } = await c.env.DB.prepare(`SELECT
       strftime('%Y-%m-%d %H:00:00', created_at) as hour,
       COUNT(*) as count
     FROM conversations
     WHERE created_at >= ?
